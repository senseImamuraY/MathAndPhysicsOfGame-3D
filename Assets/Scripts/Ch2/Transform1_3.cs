...
public class Transform1_3 : MonoBehaviour {
    ...
    void FixedUpdate()
    {
        ...
        matTransform.m00 = Mathf.Cos(fAngle); matTransform.m01 = -Mathf.Sin(fAngle);
        matTransform.m10 = Mathf.Sin(fAngle); matTransform.m11 =  Mathf.Cos(fAngle);
        transform.position = matTransform * v3InitialPos;           // 変換
        transform.rotation = qInitialRot;                           // 回転初期化
        transform.Rotate(0.0f, 0.0f, fAngle * 360.0f / (2.0f * Mathf.PI), Space.World);
//        transform.Rotate(0, 0, fAngle * Mathf.Rad2Deg, Space.World);
